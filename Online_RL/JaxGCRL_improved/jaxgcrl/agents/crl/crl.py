...
from .networks import Actor, Encoder, LearnableTemperature
...
def flatten_batch(buffer_config, transition, sample_key):
    ...
    next_state = transition.observation[1:, :state_size]  # s_{t+1} for forward-dynamics aux loss
...
class CRL:
    ...
    energy_fn: Literal["norm", "l2", "dot", "cosine", "iqe"] = "norm"

    # Idea 1: Learnable temperature (τ = exp(log_tau) rescales energy logits)
    use_learnable_temperature: bool = False
    initial_log_tau: float = 0.0

    # Idea 2: Hard negative mining (0 = disabled; >0 = keep diag + top-K hardest per row/col)
    hard_negative_top_k: int = 0

    # Idea 3: Forward-dynamics auxiliary loss coefficient (0 = disabled)
    forward_dynamics_coef: float = 0.0

    # Idea 4: IQE quasimetric energy function — number of interval components.
    # Requires repr_dim % iqe_num_components == 0.
    iqe_num_components: int = 8
    ...
    def check_config(self, config):
        ...
        if self.energy_fn == "iqe":
            assert self.repr_dim % self.iqe_num_components == 0, (
                f"IQE requires repr_dim ({self.repr_dim}) divisible by "
                f"iqe_num_components ({self.iqe_num_components})"
            )
    ...
    def train_fn(
        ...
        key, buffer_key, eval_env_key, env_key, actor_key, sa_key, g_key, temp_key = jax.random.split(key, 8)
        ...
        # Idea 1: optional learnable temperature scalar
        if self.use_learnable_temperature:
            temperature = LearnableTemperature(initial_log_tau=self.initial_log_tau)
            temperature_params = temperature.init(temp_key)
            critic_params_dict = {
