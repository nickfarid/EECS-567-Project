...
def energy_fn(name, x, y, iqe_num_components: int = 8):
    if name == "norm":
        ...
        D = x.shape[-1]
        n = D // iqe_num_components
        diff = jax.nn.relu(x - y)
        grouped = diff.reshape(*diff.shape[:-1], iqe_num_components, n)
        component_dist = jnp.max(grouped, axis=-1)
        total = jnp.sum(component_dist, axis=-1)
        return -total
    else:
        raise ValueError(f"Unknown energy function: {name}")


def _hard_neg_mask(logits, top_k: int, axis: int):
    B = logits.shape[0]
    diag = jnp.eye(B, dtype=bool)
    off = jnp.where(diag, -jnp.inf, logits)
    if axis == 1:
        topk, _ = jax.lax.top_k(off, top_k)
        threshold = topk[:, -1:]
        keep = diag | (logits >= threshold)
    else:
        topk, _ = jax.lax.top_k(off.T, top_k)
        threshold = topk[:, -1:].T
        keep = diag | (logits >= threshold)
    return jnp.where(keep, logits, -jnp.inf)


def contrastive_loss_fn(name, logits, top_k: int = 0):
    if top_k > 0:
        logits_row = _hard_neg_mask(logits, top_k, axis=1)
        logits_col = _hard_neg_mask(logits, top_k, axis=0)
    else:
        logits_row = logits
        logits_col = logits

    if name == "fwd_infonce":
        critic_loss = -jnp.mean(jnp.diag(logits) - jax.nn.logsumexp(logits_row, axis=1))
    elif name == "bwd_infonce":
        critic_loss = -jnp.mean(jnp.diag(logits) - jax.nn.logsumexp(logits_col, axis=0))
    elif name == "sym_infonce":
        critic_loss = -jnp.mean(
            2 * jnp.diag(logits)
            - jax.nn.logsumexp(logits_row, axis=1)
            - jax.nn.logsumexp(logits_col, axis=0)
        )
...
def update_actor_and_alpha(config, networks, transitions, training_state, key):
    def actor_loss(actor_params, critic_params, log_alpha, transitions, key):
        ...
        qf_pi = energy_fn(
            ...
            sa_repr,
            g_repr,
            iqe_num_components=config.get("iqe_num_components", 8),
        )

        if config.get("use_learnable_temperature", False):
            log_tau = networks["temperature"].apply(critic_params["temperature"])
            qf_pi = qf_pi / jnp.exp(log_tau)
...
def update_critic(config, networks, transitions, training_state, key):
    def critic_loss(critic_params, transitions, key):
        ...
        if config.get("use_learnable_temperature", False):
            log_tau = networks["temperature"].apply(critic_params["temperature"])
            logits = logits / jnp.exp(log_tau)

        critic_loss = contrastive_loss_fn(
            config["contrastive_loss_fn"],
            logits,
            top_k=config.get("hard_negative_top_k", 0),
        )
        ...
        # Forward-dynamics auxiliary loss (Idea 3)
        fwd_coef = config.get("forward_dynamics_coef", 0.0)
        if fwd_coef > 0:
            next_state = transitions.extras["next_state"]
            goal_indices = jnp.asarray(config["goal_indices"])
            next_goal = jnp.take(next_state, goal_indices, axis=-1)
            next_g_repr = networks["g_encoder"].apply(g_encoder_params, next_goal)
            aux = jnp.mean(
                jnp.sum((sa_repr - jax.lax.stop_gradient(next_g_repr)) ** 2, axis=-1)
            )
            critic_loss = critic_loss + fwd_coef * aux
